...
from data_utils import get_ogbn_products_with_splits, \
      get_snap_patents, get_ogbn_papers100M_with_splits, get_data_pt_file
...
def get_dataset_with_splits(dataset):
    ...
    if dataset == 'ogbn-products':
        adj, features, labels, idx_train, idx_val, idx_test = get_ogbn_products_with_splits()
    elif dataset == 'snap-patents':
        adj, features, labels, idx_train, idx_val, idx_test = get_snap_patents()
    elif dataset == 'ogbn-papers100M':
        adj, features, labels, idx_train, idx_val, idx_test = get_ogbn_papers100M_with_splits()
...
class LargeGTTokens(torch.utils.data.Dataset):
    ...
    def __init__(self, name, sample_node_len=50, seed=0):
        ...
        if os.path.exists(file_path):
            ...
            data_list = get_data_pt_file(name, get_dataset_with_splits(name.split('_')[0]), self.sample_node_len)
        ...
        try:
            ...
            self.hop2token_feats = torch.load(file_path.replace('.pt', '_hop2token_feats.pt'))
    ...
    def collate_new(self, batch, original_X):
        ...
        i, j = torch.meshgrid(torch.arange(mini_batch_size), torch.arange(self.token_len), indexing='ij')
