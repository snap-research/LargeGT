...
def train(model, loader, x, pos_enc, y, optimizer, device, conv_type, evaluator=None):
    ...
    if conv_type == 'global': 
        ...
        for seq, node_idx in loader:   
            ...
            feat = x[node_idx] if torch.is_tensor(x) else x(node_idx)
            input = seq.to(device), feat.to(device), pos_enc[node_idx].to(device), node_idx
...
def test(model, loader, x, pos_enc, y, device, conv_type, fast_eval=False, evaluator=None):
    model.eval()
    ...
    if conv_type == 'global': 
        ...
        for seq, node_idx in loader:   
            ...
            feat = x[node_idx] if torch.is_tensor(x) else x(node_idx)
            out = model.to(device)(seq.to(device), feat.to(device), pos_enc[node_idx].to(device), node_idx)
...
def main(tstamp=0):
    ...
    if args.dataset.startswith('ogbn') :
        ...
        dataset_new_tokenizer = LargeGTTokens(args.dataset+"_sample_node_len_"+str(args.sample_node_len), 
    ...
    if args.conv_type == 'local' :
        ...
        if dataset_name_input == 'ogbn-papers100M' and args.sample_node_len == 50:
            ...
            ogb_node2vec = torch.load(f'data/{dataset_name_input}_data_dict.pt', map_location='cpu') # 128 dim
            ...
            all_original_split_idx = torch.cat((split_idx['train'], split_idx['valid'], split_idx['test'])).tolist()
            ...
            pos_enc = torch.load(f'data/{dataset_name_input}_embedding_{args.global_dim}.pt', map_location='cpu')
    ...
    if args.conv_type == 'global' :
        ...
        if args.dataset == 'ogbn-papers100M' and args.sample_node_len == 100:
            ...
            custom_collate = partial(dataset_new_tokenizer.collate, original_X=original_X)
        else:
            custom_collate = dataset_new_tokenizer.collate
            
        train_loader = torch.utils.data.DataLoader(split_idx['train'], batch_size=args.batch_size, 
                                                    shuffle=True, num_workers=args.num_workers, collate_fn=custom_collate)
        valid_loader = torch.utils.data.DataLoader(split_idx['valid'], batch_size=args.test_batch_size, 
                                                    shuffle=False, num_workers=args.num_workers, collate_fn=custom_collate)
        test_loader = torch.utils.data.DataLoader(split_idx['test'], batch_size=args.test_batch_size, 
                                                    shuffle=False, num_workers=args.num_workers, collate_fn=custom_collate)
    ...
    for epoch in range(1, 1 + args.epochs):
        ...
        train_loss, train_acc = train(model, train_loader, x, pos_enc, y, optimizer, device, args.conv_type, evaluator)
        ...
        if epoch > test_start_epoch and epoch % args.test_freq == 0 :
            if args.save_ckpt:
                ...
                torch.save(ckpt, f'{args.save_path}/{args.dataset}_ckpt_epoch{epoch}.pt')
                # ckpt = model.load_state_dict(torch.load('model.pt'))

            else :
                start = time.time()
                valid_acc = test(model, valid_loader, x, pos_enc, y, device, args.conv_type, False, evaluator)
                ...
                if args.dataset == 'ogbn-products' and  valid_acc < 0.0 :
                    ...
                    test_acc = test(model, test_loader, x, pos_enc, y, device, args.conv_type, fast_eval_flag, evaluator)
